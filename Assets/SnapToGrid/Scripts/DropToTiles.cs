...
public class DropToTiles : MonoBehaviour
{
    ...
    void Setup()
    {
        m_floorTiles = FindObjectsOfType<CellStatus>(); //Find all elible tiles, must have the TileStatus script attached
    ...
    public void PlaceObject(GameObject g)
    {
        ...
        List<CellStatus> availableTiles = new List<CellStatus>();

        //Check for unoccupied tiles tagged with our tile tag
        foreach (CellStatus t in m_floorTiles)
        {
            if (t.IsOccupied == false && t.tag == m_tilesTag)
                availableTiles.Add(t);
        }

        if (availableTiles.Count >= 1)
        {
            CellStatus t = availableTiles[Random.Range(0, availableTiles.Count - 1)];
            m_canPlace = false;
    ...
    IEnumerator FallToTile(CellStatus tile, GameObject go)
    {
